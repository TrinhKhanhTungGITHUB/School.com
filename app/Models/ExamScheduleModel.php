...
class ExamScheduleModel extends Model
{
    ...
    {
        return ExamScheduleModel::where('exam_id', '=', $exam_id)->where('class_id', '=', $class_id)->where('subject_id', '=', $subject_id)
            ->first();
    }


    static public function getDeleteRecord($exam_id, $class_id, $subject_id)
    {
        return ExamScheduleModel::where('exam_id', '=', $exam_id)->where('class_id', '=', $class_id)->where('subject_id', '=', $subject_id)
            ->delete();
    }

    static public function getRecordExamClass($exam_id, $class_id)
    {
        return ExamScheduleModel::select('exam_schedule.*', 'exam.name as exam_name', 'subject.name as subject_name', 'subject.type as subject_type')
            ->join('exam', 'exam.id', '=', 'exam_schedule.exam_id')
            ->join('subject', 'subject.id', '=', 'exam_schedule.subject_id')
            ->where('exam_id', '=', $exam_id)->where('class_id', '=', $class_id)
            ->get();
    }
    ...
    static public function getExamByClass($class_id)
    {
    ...
    // Lấy kì kiểm tra của lớp có giáo viên này.
    ...
    static public function getExamName($exam_id, $class_id)
    {
        return ExamScheduleModel::select('exam.name as exam_name')
            ->join('exam', 'exam.id', '=', 'exam_schedule.exam_id')
            ->where('exam_schedule.exam_id', '=', $exam_id)
            ->where('exam_schedule.class_id', '=', $class_id)
            ->first();
    }
    static public function getExamSingleTimetable($exam_id, $class_id)
    {
        return ExamScheduleModel::select('exam_schedule.*', 'exam.name as exam_name','subject.name as subject_name', 'subject.type as subject_type')
            ->join('exam', 'exam.id', '=', 'exam_schedule.exam_id')
            ->join('subject', 'subject.id', '=', 'exam_schedule.subject_id')
